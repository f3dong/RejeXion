...
router.get("/entries/:entryId/growth-notes", requireAuth, async (req, res): Promise<void> => {
...
router.post("/entries/:entryId/growth-notes", requireAuth, async (req, res): Promise<void> => {
  const userId = req.session.userId!;
  const rawEntryId = Array.isArray(req.params.entryId) ? req.params.entryId[0] : req.params.entryId;
